...
namespace GYX.Web
{
    ...
    public class AutoBinder
    {
        ...
        /// <summary>
        /// 将对象的值填充至对象  2014-11-28  唐锋
        /// </summary>
        /// <param name="obj">数据对象</param>
        /// <param name="dataObj">插入的对象</param>
        /// <param name="flag">null 值默认为 ###</param>
        public static object BindObjectToObjectWithNoNullValues(object obj, object dataObj, string flag)
        {
            if (obj == null)
            {
                return null;
            ...
            // Type objType = obj.GetType();
            //PropertyInfo[] objypePropertiesArray = objType.GetProperties();
            //string id = obj["ID"];// objypePropertiesArray.GetValue(1).ToString();
            Type dataObjype = dataObj.GetType();
            ...
            {
                object objKeyValue = GetPropertyValue(obj, dataObjProperty.Name);
                if (objKeyValue != null)
                {
                    dataObjProperty.SetValue(dataObj, Convert.ChangeType(objKeyValue, dataObjProperty.PropertyType), null);
                }
                else
                {
                    dataObjProperty.SetValue(dataObj, Convert.ChangeType(flag, dataObjProperty.PropertyType), null);
                }
            }
            return obj;
        }
        ...
        public static object GetPropertyValue(object info, string field)
        {
            ...
            PropertyInfo[] infoTypePropertiesArray = infoType.GetProperties();
            foreach (PropertyInfo infoObjProperty in infoTypePropertiesArray)
            {
                if (infoObjProperty.Name.ToLower() == field.ToLower())
                {
                    return infoObjProperty.GetValue(info, null);
                }
            }
            return null;
        }
