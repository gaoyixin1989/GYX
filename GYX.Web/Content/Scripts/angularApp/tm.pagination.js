...
angular.module('tm.pagination', []).directive('tmPagination', [function () {
    return {
        restrict: 'EA',
        template: '<div>' +
       ' <a  ng-class="{disabled: conf.currentPage == 1}" ng-click="prevPage()" class="a-1">上一页</a>' +
        '<a  ng-class="{disabled: conf.currentPage == conf.numberOfPages}" ng-click="nextPage()" class="a-1">下一页</a>' +
        '<span>转到第<input type="text" ng-model="jumpPageNum"  />页</span>' +
        '<a  ng-click="jumpToPage($event)" class="cell-btn-blue">GO</a>' +
   ' </div> ',
        replace: true,
        scope: {
            conf: '='
